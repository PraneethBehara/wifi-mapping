...
class Explorer:
    ...
    def grid_callback(self, data):
        ...
        if self.state == BotState.READY:
            ...
            if self.botstep == BotStep.ROT:
                ...
                self.logtofile('time_ig.py', self.time)
                ...
                self.publish_frontiers(horizon_ind)
                ...
                # pydb.debugger()
                mark = self.get_max_gain(gain.copy(),horizon_ind)
                print mark
                goal = self.find_first_empty(grid.copy(), self.pose_in_ogrid(mark))
                print goal
    ...
    def get_max_gain(self, gain, finds):
        # pydb.debugger()
        value = -999999
        result = None
        for each in finds:
            if value < gain[each[0]][each[1]]:
                value = gain[each[0]][each[1]]
                result = each
        return result
