...
class Fast_localize:
    ...
    def locate_aps(self):
        ...
        if self.moves > waits:
            for each in self.Z.keys():
                ...
                x[0]=mean[0]; x[1]=mean[1]; x[2]=3.0; x[3]=35.0
                ...
                while mod_dx < threshold:
                    ...
                    covar_dx = 0.5*inv(np.dot(np.dot(H, inv(R)), np.transpose(H)))
                    ...
                    if (newx[3]<101) and range<200 and dop < prev_dop and mod_dx < prev_mod_dx and newx[2] < 7 and newx[2] > 1:
    ...
    def get_dop(self, H, R):
        store = np.trace(inv(np.dot(np.dot(H, inv(R)), np.transpose(H))))
        if store > 0: return sqrt(store)
        else : return 999
